...
// Bot UI wiring
async function refreshBotStatus(){ try{ const res = await fetch('/api/bot/status', { headers:{ Authorization:'Bearer '+localStorage.getItem('token') } }); if(!res.ok) return; const j = await res.json(); const el=$('bot-status'); if(el){ el.textContent = j && j.enabled ? `Bot: enabled (${j.config?.pair||''}, alloc €${j.config?.allocateEUR||0})` : 'Bot: disabled'; if(j && j.config){ if($('bot-pair')) $('bot-pair').value = j.config.pair || 'BTC/EUR'; if($('bot-allocate')) $('bot-allocate').value = j.config.allocateEUR || 0; if($('bot-threshold')) $('bot-threshold').value = (Number(j.config.thresholdPct||0)*100).toString(); if($('bot-lookback')) $('bot-lookback').value = j.config.lookback || 5; if($('bot-cooldown')) $('bot-cooldown').value = j.config.cooldownMs || 10000; } } }catch(e){} }

document.addEventListener('DOMContentLoaded', ()=>{
  if(location.pathname.endsWith('dashboard.html')){
    const en=$('bot-enable'), dis=$('bot-disable');
    if(en) en.addEventListener('click', async ()=>{
      const pair=$('bot-pair').value; const allocateEUR=Number($('bot-allocate').value)||0; const thresholdPct=(Number($('bot-threshold').value)||1)/100; const lookback=Number($('bot-lookback').value)||5; const cooldownMs=Number($('bot-cooldown').value)||10000;
      const res = await fetch('/api/bot/config', { method:'POST', headers:{ 'Content-Type':'application/json', Authorization:'Bearer '+localStorage.getItem('token') }, body: JSON.stringify({ pair, allocateEUR, thresholdPct, lookback, cooldownMs }) });
      if(!res.ok){ const j = await res.json().catch(()=>({})); alert('Bot config failed: '+(j.error||res.status)); } else { refreshBotStatus(); }
    });
    if(dis) dis.addEventListener('click', async ()=>{ await fetch('/api/bot/disable', { method:'POST', headers:{ Authorization:'Bearer '+localStorage.getItem('token') } }); refreshBotStatus(); });
    setInterval(refreshBotStatus, 5000);
    setTimeout(refreshBotStatus, 600);
  }
});
